...
class ReturnsBandit(BaseBandit):
    def __init__(self, args, logger):
        ...
        self.uniform_noise = th.distributions.one_hot_categorical.OneHotCategorical(th.tensor([1/self.args.noise_dim for _ in range(self.args.noise_dim)]).repeat(self.args.batch_size_run, 1))
    ...
    def sample(self, state, test_mode):
        if test_mode:
            ...
            m = th.distributions.one_hot_categorical.OneHotCategorical(probs_eps)
            action = m.sample().cpu()
            return action
    ...
    def update_returns(self, states, actions, returns, test_mode, t):
        ...
        action_distrib = th.distributions.OneHotCategorical(F.softmax(estimated_returns_all, dim=1))
    ...
    def save_model(self, path):
        th.save(self.net.state_dict(), "{}/returns_bandit_net.th".format(path))
