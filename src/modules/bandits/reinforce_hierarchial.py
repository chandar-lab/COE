...
class Z_agent(BaseBandit):
    ...
    def sample(self, state, test_mode):
        ...
        m = th.distributions.one_hot_categorical.OneHotCategorical(probs)
        action = m.sample()
        return action
    ...
    def update_returns(self, states, actions, returns, test_mode, t):
        ...
        policy_loss = -th.dot(log_probs, returns)
        policy_loss.backward()
        self.optimizer.step()


# Max entropy Z agent
class EZ_agent(BaseBandit):
    def __init__(self, args, logger):
        super().__init__(args, logger)
        self.lr = args.lr
        self.noise_dim = self.args.noise_dim
        ...
        self.uniform_distrib = th.distributions.one_hot_categorical.OneHotCategorical(th.tensor([1/self.args.noise_dim for _ in range(self.args.noise_dim)]).repeat(self.args.batch_size_run, 1))
    ...
    def update_returns(self, states, actions, returns, test_mode, t):
        ...
        for s,a,r in zip(states, actions, returns):
            self.buffer.append((s,a,th.tensor(r, dtype=th.float)))
        ...
        for _ in range(self.args.bandit_iters):
            ...
            states_ = th.stack([x[0] for x in batch_elems]).to(states.device)
            actions_ = th.stack([x[1] for x in batch_elems]).to(states.device)
            returns_ = th.stack([x[2] for x in batch_elems]).to(states.device)

            probs = self.policy(states_)
            m = th.distributions.one_hot_categorical.OneHotCategorical(probs)
            log_probs = m.log_prob(actions_.to(probs.device))
            self.optimizer.zero_grad()
            policy_loss = -th.dot(log_probs, th.tensor(returns_, device=log_probs.device).float()) + self.entropy_scaling * log_probs.sum()
            policy_loss.backward()
            self.optimizer.step()
    ...
    def save_model(self, path):
        th.save(self.policy.state_dict(), "{}/ez_bandit_policy.th".format(path))
